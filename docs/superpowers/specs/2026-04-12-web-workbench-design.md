...
- 未保存改动导航守卫：`beforeunload` 阻止关闭/刷新、页面切换前 `confirm` 确认、同页面切换文件前 `confirm` 确认
- 页面状态缓存：切换页面后切回，之前选中的文件仍被选中（`selectedPath` 通过 `pageState` 提升）
- 失败任务重试：任务卡中 `status === 'failed'` 时显示"重试"按钮，重新执行上次失败的 action
